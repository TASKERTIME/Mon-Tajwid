...
export default function Splash() {
  ...
  useEffect(() => {
    const timers = [
      ...
      setTimeout(() => router.replace(isAuthenticated ? '/' : '/auth'), 2800),
    ]
  ...
  return (
    <div className="min-h-dvh flex flex-col items-center justify-center relative overflow-hidden px-6">
      ...
      <div className={`transition-all duration-1000 ease-out ${step>=1?'opacity-100 scale-100':'opacity-0 scale-50'}`}>
        <div className="w-28 h-28 anim-float">
          <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-[0_0_30px_rgba(201,168,76,0.3)]">
      ...
      <div className={`mt-10 transition-all duration-700 ${step>=3?'opacity-100 translate-y-0':'opacity-0 translate-y-4'}`}>
        <div className="glass-gold px-6 py-3 flex items-center gap-3">
          <span className="text-2xl">🌙</span>
          <span className="text-[#c9a84c] text-sm font-medium">Profite du Ramadan pour mémoriser le Coran</span>
        </div>
      </div>

      {/* Loading */}
      <div className={`mt-10 flex gap-2 transition-opacity duration-500 ${step>=2?'opacity-100':'opacity-0'}`}>

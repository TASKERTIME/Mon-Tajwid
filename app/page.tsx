...
function DuelsTab({ username }: { username: string|null }) {
  ...
  const getLink = () => `${typeof window!=='undefined'?window.location.origin:''}/duel/${code}?s=${surah}&by=${username||'ami'}`
  const getMessage = () => `${username || 'Un ami'} te défie sur la sourate ${surah} dans Mon Tajwid ! Qui récite le mieux ? Relève le défi ici : ${getLink()}`

  const shareWhatsApp = () => { window.open(`https://wa.me/?text=${encodeURIComponent(getMessage())}`, '_blank') }
  const shareSMS = () => { window.open(`sms:?body=${encodeURIComponent(getMessage())}`, '_blank') }
  const copyLink = () => { navigator.clipboard?.writeText(getLink()); setCopied(true); setTimeout(()=>setCopied(false),2000) }
  ...
  return <div>
    ...
    {code && (
      <div className="glass p-5 anim-fade-up">
        <p className="text-[#34d399] text-xs font-semibold mb-3">Duel créé ! Envoie le défi :</p>
        <div className="flex items-center gap-2 bg-white/[0.03] rounded-xl p-3 mb-4">
          <code className="text-[#c9a84c] text-sm flex-1 font-mono truncate">{code}</code>
          <button onClick={copyLink} className="p-2 rounded-lg bg-white/5 hover:bg-white/10 transition">
        ...
        <div className="space-y-2">
          <button onClick={shareWhatsApp} className="w-full flex items-center gap-3 glass p-3.5 rounded-2xl hover:bg-white/[0.04] transition text-left">
            ...
            <div className="flex-1"><p className="text-white/60 text-sm font-medium">WhatsApp</p><p className="text-white/20 text-[11px]">Envoyer via WhatsApp</p></div>
            <ArrowRight className="w-4 h-4 text-white/10" />
          </button>

          <button onClick={shareSMS} className="w-full flex items-center gap-3 glass p-3.5 rounded-2xl hover:bg-white/[0.04] transition text-left">
            <div className="w-10 h-10 rounded-xl bg-blue-500/10 flex items-center justify-center shrink-0">
              <MessageCircle className="w-5 h-5 text-blue-400" />
            </div>
            <div className="flex-1"><p className="text-white/60 text-sm font-medium">SMS / iMessage</p><p className="text-white/20 text-[11px]">Envoyer par message</p></div>
            <ArrowRight className="w-4 h-4 text-white/10" />
          </button>

          <button onClick={copyLink} className="w-full flex items-center gap-3 glass p-3.5 rounded-2xl hover:bg-white/[0.04] transition text-left">
            <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center shrink-0">
              {copied ? <CheckCircle2 className="w-5 h-5 text-[#34d399]" /> : <Copy className="w-5 h-5 text-white/25" />}
            </div>
            <div className="flex-1"><p className="text-white/60 text-sm font-medium">{copied ? 'Copié !' : 'Copier le lien'}</p><p className="text-white/20 text-[11px]">Colle-le où tu veux</p></div>
          </button>
        </div>
      </div>
    )}
